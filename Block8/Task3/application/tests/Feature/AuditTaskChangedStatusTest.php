<?php

namespace Task3\Feature;

use Final3\App\Enums\Priority;
use Final3\App\Enums\TaskStatus;
use Final3\App\Models\AuditLogs;
use Final3\App\Models\Project;
use Final3\App\Models\Task;
use Final3\App\Models\User;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Laravel\Sanctum\Sanctum;
use Tests3\TestCase;

class AuditTaskChangedStatusTest extends TestCase
{
    use RefreshDatabase;
    public function testMain()
    {
        $user = User::factory()->create();

        $auditLogsInitialCount = AuditLogs::count();


        $project = Project::factory()->create([
            'owner_id' => $user->id . '',
            'name' => 'test'
        ]);
        Sanctum::actingAs($user);
        $taskData = [
            'title' => 'test',
            'project_id' => $project->id . '',
            'description' => 'test description',
            'status' => TaskStatus::NEW->value,
            'priority' => Priority::NORMAL->value,
            'due_date' => new \DateTimeImmutable()->format('c')
        ];
        $response = $this->postJson('/api/tasks', $taskData);

        $auditLogsAfterCreateCount = AuditLogs::count();

        $this->assertEquals($auditLogsInitialCount + 1, $auditLogsAfterCreateCount);

        $response->assertStatus(201);
        $taskFromDB = Task::where('title', $taskData['title'])
            ->where('project_id', $taskData['project_id'])
            ->where('description', $taskData['description'])
            ->where('status', $taskData['status'])
            ->where('priority', $taskData['priority'])
            ->where('due_date', $taskData['due_date'])
            ->first();
        $taskId = $taskFromDB->id;
        $newTaskData = $taskData;
        $newTaskData['status'] = TaskStatus::IN_PROGRESS->value;

        $response = $this->patchJson('/api/tasks/' . $taskId, $newTaskData);
        $response->assertStatus(200);

        $auditLogsAfterUpdateCount = AuditLogs::count();

        $this->assertDatabaseHas('audit_logs', [
            'entity_type' => Task::class,
            'entity_id' => $taskId,
            'action' => 'Changed status',
        ]);

        $this->assertEquals($auditLogsAfterCreateCount + 1, $auditLogsAfterUpdateCount);
    }
}
