<?php
namespace StorageTask6\Application\ORM;
use PDO;
use PDOException;
use StorageTask6\Application\Utils\CLHelper;
use StorageTask6\Domain\Enums\TextColorsEnum;

class ORM{

    private PDO $connection;
    private $queriesDirectory = __DIR__ . '\\..\\..\\..\\database\\queries\\';

    public function __construct(PDO $connection){
        $this->connection = $connection;
    }
    public function keysetPagination(string $queryFile, int $hookId, int $limit, int|null $lastId = null)
    {
        $result = [];
        try{
            $query = $this->getQuery($queryFile);
            $queryStatement = $this->connection->prepare($query);

            $queryStatement->bindParam(":hook_id", $hookId, PDO::PARAM_INT);
            $queryStatement->bindParam(":limit_value", $limit, PDO::PARAM_INT);

            if($lastId !== null){
                $queryStatement->bindParam(":last_id", $lastId, PDO::PARAM_INT);
            }
            else
            {
                $lastId = 999999;
                $queryStatement->bindParam(":last_id", $lastId, PDO::PARAM_INT);
            }

            $queryStatement->execute();
            $result = $queryStatement->fetchAll(PDO::FETCH_ASSOC);
        }
        catch (PDOException $e){
            CLHelper::send($e->getMessage(), TextColorsEnum::RED);
        }
        return [
          'data' => $result,
          'lastId' => emptY($result) ? null : end($result)['id']
        ];
    }
    public function offsetPagination(string $queryFile, int $hookId, int $limit, int $page): array
    {
        $result = [];
        try{
            $query = $this->getQuery($queryFile);
            $queryStatement = $this->connection->prepare($query);
            $offset = ($page - 1) * $limit;
            $queryStatement->bindParam(":limit_value", $limit, PDO::PARAM_INT);
            $queryStatement->bindParam(":offset_value", $offset, PDO::PARAM_INT);
            $queryStatement->bindParam(":hook_id", $hookId, PDO::PARAM_INT);
            $queryStatement->execute();
            $result = $queryStatement->fetchAll(PDO::FETCH_ASSOC);
        }
        catch (PDOException $e){
            CLHelper::send($e->getMessage(), TextColorsEnum::RED);
        }
        return $result;
    }
    public function selectQuery(string $queryFile, array|null $inputData = null): array
    {
        $result = [];
        try{
            $query = $this->getQuery($queryFile);
            $queryStatement = $this->connection->prepare($query);
            if($inputData !== null){
                $queryStatement->execute($inputData);
            }
            else
            {
                $queryStatement->execute();
            }
            $result = $queryStatement->fetchAll(PDO::FETCH_ASSOC);
        }
        catch (PDOException $e){
            CLHelper::send($e->getMessage(), TextColorsEnum::RED);
        }
        return $result;
    }
    public function getQuery(string $queryFile)
    {
        $allQueriesDirFilesArray = scandir($this->queriesDirectory);
        foreach ($allQueriesDirFilesArray as $key => $value) {
            if (str_ends_with($value, ".sql") === false) {
                unset($allQueriesDirFilesArray[$key]);
                continue;
            }
            if($queryFile == $value){
                return file_get_contents($this->queriesDirectory . $value, true);
            }
        }

    }
}